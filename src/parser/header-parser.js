...
var HeaderParser = /** @class */ (function () {
  ...
  HeaderParser.prototype.parse = function (content) {
    ...
    if (
      !((_a = lines[0]) === null || _a === void 0
        ? void 0
        : _a.startsWith("# speclang-header"))
    ) {
      throw new poc_1.POCError(
        "HEADER_ERROR",
        "Missing # speclang-header marker",
      );
    }
    // Extract line count
    ...
    if (!lineCountMatch) {
      throw new poc_1.POCError(
        "HEADER_ERROR",
        "Missing lines:N in header marker",
      );
    }
    ...
    if (lineCount <= 0 || lineCount > lines.length) {
      throw new poc_1.POCError(
        "HEADER_ERROR",
        "Invalid header line count: "
          .concat(lineCount, " (must be 1-")
          .concat(lines.length, ")"),
        undefined,
      );
    }
    ...
    if (
      !((_b = headerLines[headerLines.length - 1]) === null || _b === void 0
        ? void 0
        : _b.trim().startsWith("---"))
    ) {
      throw new poc_1.POCError(
  ...
  HeaderParser.prototype.parseYaml = function (yamlContent) {
    try {
      ...
      throw new poc_1.POCError(
        "HEADER_ERROR",
        "Failed to parse YAML header: ".concat(error.message),
        undefined,
        error,
      );
    }
