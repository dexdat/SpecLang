...
import { parseSpecContent } from './header';

// ============================================================================
// VALID ENUMS
// ============================================================================

const VALID_PROJECT_LEVELS: ProjectLevel[] = [
  'POC', 'MVP', 'Alpha', 'Beta', 'Production', 'Startup', 'SMB', 'MSB', 'Enterprise'
];

const VALID_AGENT_SUPPORTS: AgentSupport[] = [
...
/** Load the spec index */
let indexCache: SpecIndex | null = null;
...
export function loadSpecIndex(indexPath: string = '_index.json'): SpecIndex {
  ...
  try {
    if (fs.existsSync(indexPath)) {
      ...
      const entries: IndexEntry[] = JSON.parse(content);
...
export function validateHeaderLines(
  content: string,
  declaredLines: number | undefined,
  filepath: string
): { errors: ValidationError[]; warnings: ValidationWarning[] } {
  const errors: ValidationError[] = [];
  const warnings: ValidationWarning[] = [];
...
export function validateSpec(
  ...
  let parsed;
  try {
    parsed = parseSpecContent(content, filepath);
  } catch (e) {
    return {
      valid: false,
      filepath,
  ...
  if (parsed.metadata.lines) {
    const linesResult = validateHeaderLines(
      content,
