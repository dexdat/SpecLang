...
async function runAssemblerTests(sourceDir: string): Promise<TestResult> {
  ...
  try {
    ...
    // Auto-derive output path when missing
    const autoDerivePath = path.join(tmpDir, 'auto-derive.spec.ts.md');
    await fs.writeFile(autoDerivePath, [
      '---',
      'id: "@specs/auto-derive"',
      'version: 1.0.0',
      'target_lang: ts',
      '---',
      '# Test Spec',
      ...
      '',
      '```typescript',
      'export function greet(): string { return "auto-derived"; }',
      '```',
    ].join('\n'), 'utf-8');
    const autoResult = await assembler.assemble(autoDerivePath);
    assert(autoResult.success === true, 'assembler: auto-derive succeeds when output missing');
    assert(autoResult.warnings.some((w: string) => w.includes('Auto-derived output')), 'assembler: warning mentions auto-derived output');
