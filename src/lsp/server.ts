...
import { parseReferences, resolveReference, resolveFileRef, findBlockInFile } from './references.js';
 ...
 */
function validateDiagnostics(textDocument: TextDocument, workspaceRoot: string): Diagnostic[] {
  ...
  // Validate @ref: annotations
  const refDiagnostics = validateReferences(text, workspaceRoot);
...
/**
 * Validate @ref: annotations in a spec document and produce diagnostics for broken references.
 */
function validateReferences(text: string, workspaceRoot: string): Diagnostic[] {
  const diagnostics: Diagnostic[] = [];
...
export function startServer(): void {
  ...
  documents.onDidOpen((event) => {
    const diagnostics = validateDiagnostics(event.document, workspaceRoot);
