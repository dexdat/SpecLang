...
import type { SpecRecord, SessionRecord } from '../db/types';
...
export interface OpenCodeEvents {
  ...
  on(event: 'session.idle', handler: (session: SessionIdleEvent) => void);
  on(event: 'session.started', handler: (session: SessionStartedEvent) => void);
  on(event: 'write.attempt', handler: (attempt: WriteAttemptEvent) => void);
