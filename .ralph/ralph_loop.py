...
class WorkflowEngine:
    ...
    def run_standard_loop(self, use_agents: bool = False, auto_commit: bool = False):
        ...
        Args:
            use_agents: If True, use speclang-simulator/speclang-simulator-verify agents instead of prompts
        ...
        while self.tasks.has_pending_tasks():
            ...
            # Build phase
            if use_agents:
                success = self.run_phase("build", "agent", "speclang-simulator", "build", auto_commit=auto_commit)
            else:
            ...
            # Verify phase
            if use_agents:
                success = self.run_phase("verify", "agent", "speclang-simulator-verify", "verify", auto_commit=auto_commit)
            else:
                success = self.run_phase(
                    "verify",
                    "prompt",
...
class RalphCLI:
    ...
    def cmd_build(self):
        """Run single build phase using speclang-simulator agent."""
        self.logger.info("Running single build phase with speclang-simulator agent...")
        self.engine.run_phase("build", "agent", "speclang-simulator", "build", auto_commit=self.auto_commit)
    
    def cmd_verify(self):
        """Run single verify phase using speclang-simulator-verify agent."""
        self.logger.info("Running single verify phase with speclang-simulator-verify agent...")
        self.engine.run_phase("verify", "agent", "speclang-simulator-verify", "verify", auto_commit=self.auto_commit)
    
    def cmd_loop(self):
        """Run full loop using speclang-simulator and speclang-simulator-verify agents."""
        self.logger.info("Starting agent-based Ralph loop with speclang-simulator and speclang-simulator-verify...")
        if self.auto_commit:
