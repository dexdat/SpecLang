...
| PITFALL-WORKFLOW-001 | Implement workflow command stubs (converge/commit, rollback, pipeline run, registry download) — **7 TODOs confirmed active as of tick #20** (DuckBrain claim of resolution by tick #10 was stale/fabricated). 4 in commands.ts, 3 in conversation.ts. | Medium | 5 | — | ++code-generation, +architecture | GLM-5.2 | 7 TODOs across 2 files; bounded implementation with clear spec references | DeepSeek V4 Pro |
