...
import { createSpec, SQLITE_TREE_QUERIES, GIT_IGNORE_RULES } from '../src/directory/creator.js';
...
describe('Directory Structure System', () => {
  ...
  describe('Default Patterns', () => {
    it('should have default directory pattern', () => {
      ...
      expect(DEFAULT_DIRECTORY_PATTERN.nesting).toContain('auth.dir/login.dir/');
    });
    
    it('should have default naming rules', () => {
      expect(DEFAULT_NAMING_RULES.spec_files).toContain('lowercase with hyphens');
      expect(DEFAULT_NAMING_RULES.spec_dirs).toContain('auth.spec.md → auth.dir/');
    });
  ...
  describe('Directory Scanning', () => {
    ...
    it('should identify spec file types', async () => {
      ...
      const handlerSpec = result.specFiles.find(f => f.name === 'handler.go.spec');
