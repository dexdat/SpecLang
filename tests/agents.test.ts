...
import type { AgentRole, Agent, Session, Task, Tool } from '../src/agents/types.js';
...
describe('Agent Session Manager', () => {
  describe('SessionManager', () => {
    ...
    it('should create a session for each agent type', () => {
      const roles: AgentRole[] = ['north-star', 'spec-writer', 'code-gen', 'test-writer', 'back-sync'];
    ...
    it('should queue tasks', () => {
      ...
      const task = sessionManager.queueTask(session.id, 'expand', 'specs/auth.spec.md', 'high');
    ...
    it('should sort tasks by priority', () => {
      ...
      sessionManager.queueTask(session.id, 'expand', 'specs/b.spec.md', 'urgent');
      sessionManager.queueTask(session.id, 'expand', 'specs/c.spec.md', 'normal');
    ...
    it('should start tasks', () => {
      const session = sessionManager.create('spec-writer');
      const task = sessionManager.queueTask(session.id, 'expand', 'specs/test.spec.md');
  ...
  describe('OwnershipRegistry', () => {
    ...
    it('should get owner for nested spec files', () => {
      ...
      expect(ownership.getOwner('specs/nested/deep/spec.spec.md')).toBe('spec-writer');
    });
    ...
    it('should check write permissions', () => {
      ...
      const canWrite = ownership.canWrite('test-agent', 'spec-writer', 'specs/nested/test.spec.md');
      expect(canWrite.allowed).toBe(true);
      expect(canWrite.owner).toBe('spec-writer');

      // spec-writer cannot write to src
      const cannotWrite = ownership.canWrite('test-agent', 'spec-writer', 'src/test.ts');
