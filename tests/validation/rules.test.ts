...
describe('Validation Rules', () => {
  describe('Header Rule', () => {
    ...
    it('should fail invalid project_level', () => {
      ...
      const results = headerRule.check(spec);
      const errors = results.filter(r => r.level === 'error');
      expect(errors.some(e => e.message.includes('project_level'))).toBe(true);
    });
  ...
  describe('Blocks Rule', () => {
    it('should pass valid blocks', () => {
      const spec: ParsedSpec = {
        ...
        content: '# Content',
        blocks: [
          { id: '@block:auth/login', kind: 'operation', content: 'Login', line: 15 },
          { id: '@block:auth/user', kind: 'entity', content: 'User entity', line: 20 },
        ],
        references: [],
        headerRaw: '',
      };
    ...
    it('should fail duplicate block IDs', () => {
      const spec: ParsedSpec = {
        ...
        blocks: [
          { id: '@block:auth/login', kind: 'operation', content: 'Login', line: 15 },
          { id: '@block:auth/login', kind: 'operation', content: 'Login 2', line: 20 },
        ],
        references: [],
        headerRaw: '',
      };
    ...
    it('should fail invalid block kind', () => {
      const spec: ParsedSpec = {
        ...
        content: '# Content',
        blocks: [
          { id: '@block:auth/login', kind: 'invalid_kind' as any, content: 'Login', line: 15 },
        ],
        references: [],
        headerRaw: '',
      };
      ...
      const results = blocksRule.check(spec);
      const errors = results.filter(r => r.level === 'error');
      expect(errors.some(e => e.message.includes('Invalid block kind'))).toBe(true);
    });
  ...
  describe('Autonomous Rule', () => {
    ...
    it('should warn on operation without step-by-step', () => {
      const spec: ParsedSpec = {
        ...
        content: '# Content',
        blocks: [
          { id: '@block:auth/login', kind: 'operation', content: 'Login here', line: 15 },
        ],
        references: [],
        headerRaw: '',
      };
      ...
      const results = autonomousRule.check(spec);
      const warnings = results.filter(r => r.level === 'warning');
      expect(warnings.some(e => e.message.includes('step-by-step'))).toBe(true);
    });
  });
