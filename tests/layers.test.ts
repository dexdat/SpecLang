...
describe("Layer System", () => {
  describe("Layer Enum", () => {
    ...
    it("should have correct layer names", () => {
      ...
      expect(LAYER_NAMES[Layer.PIPELINE_CONFIGURATION]).toBe(
  ...
  describe("Layer Owners", () => {
    ...
    it("should assign spec-writer to component architecture", () => {
      expect(LAYER_OWNERS[Layer.COMPONENT_ARCHITECTURE]).toContain(
        "spec-writer",
      );
    });
  ...
  describe("getMinValidLayer", () => {
    ...
    it("should return minimum layer from dependencies", () => {
      expect(
        getMinValidLayer([Layer.DOMAIN_MODELS, Layer.COMPONENT_ARCHITECTURE]),
      ).toBe(Layer.COMPONENT_ARCHITECTURE);
    });
  ...
  describe("validateLayer", () => {
    ...
    it("should fail for undefined layer", () => {
      ...
      expect(result.errors).toContain(
        LAYER_VALIDATION_RULES.LAYER_FIELD_REQUIRED,
      );
    });
  ...
  describe("validateLayerDependency", () => {
    it("should allow lower layer dependencies", () => {
      ...
      const result1 = validateLayerDependency(
        Layer.CODE_MAPPING,
        Layer.NORTH_STAR,
      );
      expect(result1.valid).toBe(true);

      const result2 = validateLayerDependency(
        Layer.CODE_MAPPING,
    ...
    it("should allow same layer dependencies", () => {
      const result = validateLayerDependency(
        Layer.BUSINESS_LOGIC,
        Layer.BUSINESS_LOGIC,
      );
      expect(result.valid).toBe(true);
  ...
  describe("validateLayerChain", () => {
    ...
    it("should warn if chain does not include NORTH_STAR", () => {
      ...
      expect(result.warnings).toContain(
        "Dependency chain does not include layer 0 (North Star).",
      );
    });
  });
  ...
  describe("getMaxLayerForMaturity", () => {
    it("should return correct max layer for each maturity level", () => {
      expect(getMaxLayerForMaturity("POC")).toBe(
        Layer.INTERFACE_SPECIFICATIONS,
      );
      ...
      expect(getMaxLayerForMaturity("PRODUCTION")).toBe(
        Layer.PIPELINE_CONFIGURATION,
      );
    });
  ...
  describe("resolveLayer", () => {
    it("should resolve valid layer numbers", () => {
      ...
      expect(resolveLayer({ explicitLayer: 9 })).toBe(
        Layer.PIPELINE_CONFIGURATION,
      );
    });
  ...
  describe("resolveLayerFromPath", () => {
    it("should resolve layer from file path patterns", () => {
      ...
      // Test Specs
      expect(resolveLayerFromPath("handler.test.spec.md")).toBe(
        Layer.TEST_SPECIFICATIONS,
      );

      // Pipeline
      expect(resolveLayerFromPath("pipeline.yaml")).toBe(
        Layer.PIPELINE_CONFIGURATION,
      );
    });
  ...
  describe("resolveLayerFromContent", () => {
    it("should resolve layer from content keywords", () => {
      expect(resolveLayerFromContent("northstar or intent")).toBe(
        Layer.NORTH_STAR,
      );
      expect(resolveLayerFromContent("component and service")).toBe(
        Layer.COMPONENT_ARCHITECTURE,
      );
      expect(resolveLayerFromContent("interface and api")).toBe(
        Layer.INTERFACE_SPECIFICATIONS,
      );
      expect(resolveLayerFromContent("business and workflow")).toBe(
        Layer.BUSINESS_LOGIC,
      );
    });
