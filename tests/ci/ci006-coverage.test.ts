...
function run(cmd: string, args: string[], cwd: string): Promise<{ stdout: string; stderr: string; status: number }> {
  return new Promise((resolve) => {
    execFile(cmd, args, { cwd, encoding: 'utf8', maxBuffer: 32 * 1024 * 1024 }, (err, stdout, stderr) => {
      resolve({
        stdout: stdout ?? '',
        stderr: stderr ?? '',
...
describe('CI-006: test coverage reporting', () => {
  ...
  beforeAll(() => {
    ...
    if (existsSync(join(REPO_ROOT, 'coverage'))) {
      prevCoverage = mkdtempSync(join(REPO_ROOT, '.tmp', 'ci006-coverage-prev-'));
      rmSync(prevCoverage, { recursive: true, force: true });
      execSync(`mv "${join(REPO_ROOT, 'coverage')}" "${prevCoverage}"`, { encoding: 'utf8' });
    }
  });
  ...
  afterAll(() => {
    if (prevCoverage) {
      ...
      execSync(`mv "${prevCoverage}" "${join(REPO_ROOT, 'coverage')}"`, { encoding: 'utf8' });
    }
  });
  ...
  it('AC1: package.json declares the test:coverage script using --coverage', () => {
    ...
    expect(script, 'test:coverage script must exist in package.json').toBeTruthy();
  ...
  it('AC2: @vitest/coverage-v8 is installed so the v8 provider resolves', () => {
    ...
    const coverageV8Path = join(REPO_ROOT, 'node_modules', '@vitest', 'coverage-v8');
    expect(existsSync(coverageV8Path), '@vitest/coverage-v8 must be installed in node_modules').toBe(true);
  });
  ...
  it('AC4: npm run test:coverage --coverage writes coverage/coverage-final.json AND coverage/index.html', async () => {
    ...
    const result = await run(
      'npx',
      [
        'vitest', 'run', 'tests/header-validation.test.ts',
        '--coverage',
    ...
    expect(existsSync(join(covDir, 'coverage-final.json')), 'coverage/coverage-final.json must exist').toBe(true);
    expect(existsSync(join(covDir, 'index.html')), 'coverage/index.html must exist').toBe(true);
    ...
    const json = JSON.parse(readFileSync(join(covDir, 'coverage-final.json'), 'utf8')) as unknown;
  ...
  it('AC7: ci.yml writes a coverage summary table to $GITHUB_STEP_SUMMARY', () => {
    ...
    for (const key of ['lines', 'statements', 'functions', 'branches']) {
      expect(wf, `summary must mention '${key}' metric`).toMatch(new RegExp(`\\b${key}\\b`));
    }
  });
  ...
  it('AC8: coverage steps appear AFTER Run tests and BEFORE the lint step', () => {
    ...
    expect(coverageIdx, 'must have a "Generate coverage report" step').toBeGreaterThan(-1);
