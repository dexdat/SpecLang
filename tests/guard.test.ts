...
describe("Guard System", () => {
  ...
  describe("OwnershipRegistry", () => {
    ...
    test("should check canWrite correctly", () => {
      ...
      expect(registry.canWrite("spec-writer", "project.scl").allowed).toBe(
        false,
      );

      // Spec writer can write spec files
      expect(
        registry.canWrite("spec-writer", "specs/auth.spec.md").allowed,
      ).toBe(true);
    });
  ...
  describe("WriteInterceptor", () => {
    test("should intercept writes correctly", async () => {
      ...
      // North star can write project.scl
      const result = await interceptor.interceptWrite(
        "north-star",
        "project.scl",
      );
      ...
      // Spec writer cannot write project.scl
      const blocked = await interceptor.interceptWrite(
        "spec-writer",
        "project.scl",
      );
    ...
    test("should intercept delete operations", async () => {
      ...
      const result = await interceptor.interceptDelete(
        "spec-writer",
        "project.scl",
      );
    ...
    test("should intercept rename operations", async () => {
      ...
      const result = await interceptor.interceptRename(
        "spec-writer",
        "project.scl",
        "project2.scl",
      );
  ...
  describe("Test Cases from Requirements", () => {
    ...
    test("6. Override rules work", () => {
      ...
      expect(registry.canWrite("code-gen", "specs/temp.spec.md").allowed).toBe(
        true,
      );
    });
