...
---
`;
    expect(() => parseHeader(noHeader)).toThrow('No speclang-header declaration found');
  });
...
---
`;
    expect(() => parseHeader(noVersion)).toThrow('Missing required field: version');
  });
...
---
`;
    expect(() => parseHeader(invalidYaml)).toThrow('Failed to parse header YAML');
  });
});
...
describe('extractReferences', () => {
  it('should extract references from depends_on', () => {
    ...
    const depRefs = result.references.filter(r => 
      r.sourceFile === undefined || r.line === 1
    );
