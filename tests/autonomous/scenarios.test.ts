...
describe('AUTONOMOUS_SCENARIOS', () => {
  ...
  it('should include spec_generation scenario', () => {
    const hasSpecGen = AUTONOMOUS_SCENARIOS.some(s => s.scenario.type === 'spec_generation');
    expect(hasSpecGen).toBe(true);
  });
  
  it('should include code_generation scenario', () => {
    const hasCodeGen = AUTONOMOUS_SCENARIOS.some(s => s.scenario.type === 'code_generation');
    expect(hasCodeGen).toBe(true);
  });
  
  it('should include cascade scenario', () => {
    const hasCascade = AUTONOMOUS_SCENARIOS.some(s => s.scenario.type === 'cascade');
    expect(hasCascade).toBe(true);
  });
  
  it('should include pipeline scenario', () => {
    const hasPipeline = AUTONOMOUS_SCENARIOS.some(s => s.scenario.type === 'pipeline');
    expect(hasPipeline).toBe(true);
  });
  
  it('should include self_specifying scenario', () => {
    const hasSelfSpec = AUTONOMOUS_SCENARIOS.some(s => s.scenario.type === 'self_specifying');
...
describe('validateScenarioConfig', () => {
  ...
  it('should reject scenario without name', () => {
    const result = validateScenarioConfig({
      ...
      expected: { success: true, metrics: { time: 1000, memory: 100, accuracy: 1 }, artifacts: [] },
      timeout: 1000
    });
