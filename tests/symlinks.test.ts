...
describe('Symlinks Module', () => {
  ...
  describe('scanSymlinks', () => {
    it('should find all symlinks in directory tree', async () => {
      ...
      await fs.symlink(path.join(physicalDir, 'a.txt'), path.join(logicalDir, 'a.txt'));
      await fs.symlink(path.join(physicalDir, 'b.txt'), path.join(logicalDir, 'b.txt'));
  ...
  describe('verifySymlinks', () => {
    it('should categorize symlinks as valid, broken, or missing', async () => {
      ...
      const symlinks: SymlinkEntry[] = [
        { logicalPath: path.join(logicalDir, 'valid.txt'), physicalPath: path.join(physicalDir, 'valid.txt'), isValid: false },
        { logicalPath: path.join(logicalDir, 'broken.txt'), physicalPath: path.join(physicalDir, 'broken.txt'), isValid: false },
        { logicalPath: path.join(logicalDir, 'missing.txt'), physicalPath: path.join(physicalDir, 'missing.txt'), isValid: false },
      ];
  ...
  describe('repairSymlinks', () => {
    it('should repair broken symlinks', async () => {
      ...
      const symlinks: SymlinkEntry[] = [
        { logicalPath: path.join(logicalDir, 'test.txt'), physicalPath: path.join(physicalDir, 'test.txt'), isValid: false },
      ];
  ...
  describe('getPhysicalPath', () => {
    ...
    it('should return null for missing files', async () => {
      const resolved = await getPhysicalPath(path.join(testDir, 'nonexistent.txt'));
  ...
  describe('rebuild', () => {
    ...
    it('should handle clean option in test directory', async () => {
      ...
      try {
        ...
        await expect(fs.readFile('src/existing.txt', 'utf-8')).rejects.toThrow();
      } finally {
