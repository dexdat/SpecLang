...
describe("Project Layout Module", () => {
  ...
  describe("initProject", () => {
    ...
    it("should create initial spec and test files", async () => {
      ...
      expect(
        fs.existsSync(
          path.join(projectRoot, "tests", "my-awesome-app.test.spec.scl"),
        ),
      ).toBe(true);
    });
  });
  ...
  describe("validateProject", () => {
    ...
    it("should return error for non-existent project", () => {
      ...
      expect(result.issues.some((i) => i.code === "NO_PROJECT_ROOT")).toBe(
        true,
      );
    });
    ...
    it("should return error when missing north star", () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.issues.some((i) => i.code === "MISSING_NORTH_STAR")).toBe(
        true,
      );
    });
    ...
    it("should return warnings for missing optional dirs", async () => {
      ...
      fs.rmSync(path.join(testDir, "minimal-project", "generated"), {
        recursive: true,
      });
      ...
      expect(
        result.issues.some((i) => i.code === "MISSING_GENERATED_DIR"),
      ).toBe(true);
    });
  });
