...
`,
  ...
  it("should generate spec from TypeScript file", async () => {
    const result = await generator.generateFromTypeScript("src/test/example.ts");
  ...
  it("should extract interface as entity block", async () => {
    ...
    expect(result.success).toBe(true);
    const hasInterface = result.spec?.blocks.some(b => b.id === "testinterface");
  ...
  it("should extract function as code block", async () => {
    ...
    expect(result.success).toBe(true);
    const hasFunction = result.spec?.blocks.some(b => b.id === "testfunction");
