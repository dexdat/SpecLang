...
describe('workflow/conversation — handleModifyConfig', () => {
  ...
  it('returns an error when .speclangrc does not exist', async () => {
    const out = await executeParsedCommand(cmd('modify_config', ['postgres'], 'use postgres'), project);
  ...
  it('switches the db setting to postgresql (normalised alias)', async () => {
    ...
    fs.writeFileSync(cfgPath, JSON.stringify({ db: 'sqlite', port: 3000 }, null, 2));
    const out = await executeParsedCommand(cmd('modify_config', ['postgres'], 'use postgres'), project);
  ...
  it('switches the db setting to sqlite', async () => {
    ...
    const out = await executeParsedCommand(cmd('modify_config', ['sqlite'], 'switch to sqlite'), project);
  ...
  it('applies a key=value override and coerces numeric values', async () => {
    ...
    const out = await executeParsedCommand(cmd('modify_config', ['port=5432'], 'port=5432'), project);
  ...
  it('keeps string values as strings in key=value overrides', async () => {
    ...
    await executeParsedCommand(cmd('modify_config', ['host=localhost'], 'host=localhost'), project);
    const updated = JSON.parse(fs.readFileSync(cfgPath, 'utf-8'));
  ...
  it('returns guidance when no entities are recognised', async () => {
    fs.writeFileSync(path.join(project, '.speclangrc'), JSON.stringify({}, null, 2));
    const out = await executeParsedCommand(cmd('modify_config', ['banana'], 'use banana'), project);
    expect(out).toMatch(/didn't recognise/i);
    // File must NOT have been modified.
    expect(fs.readFileSync(path.join(project, '.speclangrc'), 'utf-8').trim()).toBe('{}');
  });
  ...
  it('handles multiple db entities in one command', async () => {
    ...
    await executeParsedCommand(cmd('modify_config', ['sqlite', 'mysql'], 'use sqlite mysql'), project);
...
describe('workflow/conversation — handleFixIssue', () => {
  ...
  it('asks for clarification when no entities are given', async () => {
    const out = await executeParsedCommand(cmd('fix_issue', [], 'fix it'), project);
  ...
  it('reports when no matching spec exists', async () => {
    ...
    const out = await executeParsedCommand(cmd('fix_issue', ['nonexistent'], 'fix nonexistent bug'), project);
  ...
  it('finds a matching spec and quotes the implicated section', async () => {
    ...
    const out = await executeParsedCommand(cmd('fix_issue', ['auth', 'login'], 'fix the auth login bug'), project);
  ...
  it('still returns the spec path when no section body mentions the entity', async () => {
    ...
    const out = await executeParsedCommand(cmd('fix_issue', ['auth'], 'fix auth'), project);
    expect(out).toContain('Relevant spec:');
...
describe('workflow/conversation — module integrity', () => {
  it('imports without throwing and exposes executeParsedCommand', async () => {
    const mod = await import('../../specs/workflow.spec.dir/src/conversation.js');
