...
class PerformanceMonitor {
   ...
   */
  checkRegression(name: string, currentValue: number, thresholdPercent: number = 20): boolean {
   ...
   */
  saveBaseline(name: string, stats: {
    mean_ms: number;
    p90_ms: number;
    p95_ms: number;
    p99_ms: number;
  ...
  private loadBaselines(): void {
    const baselinesDir = path.join(process.cwd(), 'tests/performance/baselines');
  ...
  generateReport(): string {
    ...
    this.baselines.forEach((baseline, name) => {
      ...
      if (current.length > 0) {
        ...
        const diff = ((mean - baseline.mean_ms) / baseline.mean_ms * 100).toFixed(1);
