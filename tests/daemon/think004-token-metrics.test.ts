 ...
 */
function makeMetricsExecutor(
  metrics: InvocationMetrics
): AgentExecutorFn & {
  calls(): Array<{
    agent: string; trigger: string; params?: Record<string, unknown>;
  }>;
} {
  const calls: Array<{
    agent: string; trigger: string; params?: Record<string, unknown>;
...
describe('THINK-004 — token accounting metrics', () => {
  ...
  describe('AgentInvoker — metrics pass-through', () => {
    ...
    it('returns undefined metrics when executor provides none', async () => {
      const executor = makeMetricsExecutor({ input_tokens: 0, output_tokens: 0 } as any);
    ...
    it('invokeMany preserves metrics per option', async () => {
      const lowMetrics: InvocationMetrics = { input_tokens: 100, output_tokens: 50 };
      const highMetrics: InvocationMetrics = { input_tokens: 500, output_tokens: 200 };
  ...
  describe('CascadeCoordinator — metrics collation', () => {
    it('populates metrics_summary on result when options.metrics is true', async () => {
      writeIndex(indexPath, [
        ...
        { id: 'specs/child-a', layer: 2, type: 'code', file: 'src/child-a.ts', depends_on: ['specs/root'] },
        { id: 'specs/child-b', layer: 2, type: 'test', file: 'tests/child-b.test.ts', depends_on: ['specs/root'] },
      ]);
      ...
      const executor = makePerAgentExecutor((agent) => metricsByAgent[agent] ?? { input_tokens: 0, output_tokens: 0 });
    ...
    it('does not populate metrics_summary when options.metrics is false (default)', async () => {
      ...
      const executor = makeMetricsExecutor({ input_tokens: 100, output_tokens: 50 });
      const coordinator = new CascadeCoordinator(indexPath, {
        skipTests: true,
        parallel: false,
    ...
    it('aggregates multiple invocations of the same agent', async () => {
      writeIndex(indexPath, [
        { id: 'specs/root', layer: 1, file: 'specs/root.spec.md' },
        { id: 'specs/a', layer: 2, type: 'code', file: 'src/a.ts', depends_on: ['specs/root'] },
        { id: 'specs/b', layer: 2, type: 'code', file: 'src/b.ts', depends_on: ['specs/root'] },
      ]);
    ...
    it('handles empty cascade with no metrics gracefully', async () => {
      ...
      const executor: AgentExecutorFn = async () => ({ success: true, files: [] });
      const coordinator = new CascadeCoordinator(indexPath, {
        skipTests: true,
        parallel: false,
        metrics: true,
    ...
    it('collateMetrics returns a standalone MetricsSummary', () => {
      ...
      const coordinator = new CascadeCoordinator(indexPath, { skipTests: true });
