...
describe('THINK-003 — cascade --thinking parser', () => {
  it('normalizes phase aliases into CoordinatorOptions agent keys', () => {
    expect(
      parseThinkingOverrides('code_generate:high,spec_read:none')
    ).toEqual({
...
describe('THINK-003 — agent runtime', () => {
  it('selects the cascade agent from --trigger when no agent is explicit', () => {
    expect(selectAgentForTrigger('specs/foo.spec.md')).toBe('speclang-spec-writer');
    expect(selectAgentForTrigger('src/foo.ts')).toBe('speclang-code-gen');
    expect(selectAgentForTrigger('tests/foo.test.ts')).toBe('speclang-test-writer');
  ...
  it('applies the adapter to the real API request boundary', async () => {
    const fetchFn = vi.fn(async (_url: string, init: RequestInit) => {
      ...
      return new Response(
        JSON.stringify({ choices: [{ message: { content: 'agent response' } }] }),
  ...
  it('exposes agent and cascade thinking flags in the public CLI', () => {
    const agentHelp = execFileSync('node', ['bin/speclang', 'agent', '--help'], {
      cwd: REPO_ROOT,
      encoding: 'utf-8',
    });
    const cascadeHelp = execFileSync('node', ['bin/speclang', 'cascade', '--help'], {
      cwd: REPO_ROOT,
      encoding: 'utf-8',
    });
