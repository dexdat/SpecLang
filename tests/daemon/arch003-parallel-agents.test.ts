...
import {
  DependencyTracker,
  type TreeNode,
} from "../../specs/cascade.spec.dir/src/coordinator/dependency.ts";
...
describe("ARCH-003 — parallel agent execution (swarm)", () => {
  ...
  describe("AgentInvoker.invokeMany()", () => {
    ...
    it("records duration_ms per invocation", async () => {
      ...
      const results = await invoker.invokeMany([{ agent: "a", trigger: "x" }]);
  ...
  describe("DependencyTracker.partitionByDepth()", () => {
    ...
    it("returns single wave when all nodes share a layer", () => {
      ...
      const nodes = ["a", "b", "c"].map((id) =>
        (tracker as any).graph.nodes.get(id),
      );
    ...
    it("runs 5 independent specs in parallel — wall time ≤ slowest", async () => {
      ...
      writeIndex(indexPath, [
        { id: "specs/root", layer: 1, file: "specs/root.spec.md" },
        {
          id: "specs/x1",
          layer: 2,
          file: "specs/x1.spec.md",
          depends_on: ["specs/root"],
        },
        {
          id: "specs/x2",
          layer: 2,
          file: "specs/x2.spec.md",
          depends_on: ["specs/root"],
        },
        {
          id: "specs/x3",
          layer: 2,
          file: "specs/x3.spec.md",
          depends_on: ["specs/root"],
        },
        {
          id: "specs/x4",
          layer: 2,
          file: "specs/x4.spec.md",
          depends_on: ["specs/root"],
        },
        {
          id: "specs/x5",
          layer: 2,
          file: "specs/x5.spec.md",
          depends_on: ["specs/root"],
        },
      ]);
    ...
    it("fans out across multiple waves by layer", async () => {
      ...
      writeIndex(indexPath, [
        { id: "specs/common", layer: 0, file: "specs/common.spec.md" },
        {
          id: "specs/a1",
          layer: 1,
          file: "specs/a1.spec.md",
          depends_on: ["specs/common"],
        },
        {
          id: "specs/a2",
          layer: 1,
          file: "specs/a2.spec.md",
          depends_on: ["specs/common"],
        },
        {
          id: "specs/b1",
          layer: 2,
          file: "specs/b1.spec.md",
          depends_on: ["specs/a1"],
        },
        {
          id: "specs/b2",
          layer: 2,
          file: "specs/b2.spec.md",
          depends_on: ["specs/a1"],
        },
        {
          id: "specs/b3",
          layer: 2,
          file: "specs/b3.spec.md",
          depends_on: ["specs/a1"],
        },
      ]);
