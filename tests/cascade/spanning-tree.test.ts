...
function hasGraphCycle(edges: DependencyEdge[]): boolean {
  ...
  for (const e of edges) { allNodes.add(e.from); allNodes.add(e.to); }
...
function buildSpanningTree(rootId: string, edges: DependencyEdge[], allNodes: Map<string, TreeNode>): SpanningTreeNode {
...
function hasCycleInTree(node: SpanningTreeNode, visited: Set<string> = new Set()): boolean {
...
function flattenTree(node: SpanningTreeNode, acc: SpanningTreeNode[] = []): SpanningTreeNode[] {
  acc.push(node);
  for (const child of node.children) {
...
describe('CycleDetector', () => {
  ...
  test('findRepeatingPattern detects ABAB pattern', () => {
    ...
    expect(result.hasCycle).toBe(true);
    expect(result.reasons.some(r => r.startsWith('Pattern detected:'))).toBe(true);
  });
...
describe('CascadeDepthManager (integration)', () => {
  ...
  test('onFileChange blocks after max depth reached (depth 5, file 5 blocked)', () => {
    const m = new CascadeDepthManager({ max_depth: 5, max_files_per_cascade: 100, quiet_period_ms: 5000 });
  ...
  test('onFileChange detects cycles from repeated file edits', () => {
    const m = new CascadeDepthManager({ max_depth: 100, max_files_per_cascade: 100, quiet_period_ms: 5000 });
    m.startCascade('cascade-1');
...
describe('Spanning Tree Construction', () => {
  function makeNode(id: string, filePath: string): TreeNode {
    return { id, layer: 0, type: 'spec' as const, filePath, dependencies: [], children: [] };
  }
  ...
  test('cycle detection works with depth tracker integration', async () => {
    const tracker = new DepthTracker({ max_depth: 100, max_files_per_cascade: 100 });
    const CycleDetectorCtor = (await import('../../src/cascade/depth/cycle-detection.js')).CycleDetector;
    const detector = new CycleDetectorCtor({ max_repeats: 3, max_pattern_length: 5 });
