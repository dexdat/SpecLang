...
// Test config
import { loadConfig, getArg, getArgInt, getArgBool } from '../../src/mcp/config.js';
...
describe('MCPAuth', () => {
  ...
  describe('token auth', () => {
    it('should return isEnabled as true when enabled', () => {
      const auth = createAuth({ enabled: true, type: 'token', token: 'test-token' });
  ...
  describe('basic auth', () => {
    it('should return isEnabled as true when enabled', () => {
      const auth = createAuth({ enabled: true, type: 'basic', user: 'admin', pass: 'secret' });
      expect(auth.isEnabled()).toBe(true);
    });
...
describe('MCPTypes', () => {
  ...
  it('should have correct LockInput type', () => {
    const input: LockInput = { resource: 'specs/test.md', agent_id: 'agent-1', ttl: 60 };
  ...
  it('should have correct IndexRefreshResult type', () => {
    const result: IndexRefreshResult = { specs_indexed: 10, refs_found: 50, errors: [] };
