...
describe('Cascade Control Interactions', () => {
  const simulateCascadeControl = () => {
    ...
    return {
      getState: () => ({ status, canPause, canFinalize, canAbort, currentFile }),
...
describe('Spec Editor Interactions', () => {
  const simulateSpecEditor = () => {
    ...
    let isDirty = false;
    let validationErrors: Array<{ line: number; column: number; message: string; severity: string }> = [];
    let previewContent = '';

    return {
      getState: () => ({ currentSpec, isDirty, validationErrors, previewContent }),
      createNewSpec: vi.fn(() => {
        currentSpec = { id: 'new-spec', content: '# speclang-header lines:5\nid: @specs/new\nversion: 0.1.0\nlayer: 5' };
        isDirty = true;
      }),
...
describe('Git Integration Interactions', () => {
  const simulateGitIntegration = () => {
    let status: { modified: string[]; staged: string[]; branch: string } | null = null;
    ...
    return {
      ...
      fetchStatus: vi.fn(() => {
        status = { modified: ['specs/test.spec.md'], staged: [], branch: 'main' };
      }),
