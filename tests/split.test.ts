...
describe("Splitter", () => {
  ...
  it("should not split small spec", () => {
    ...
    const result = splitter.split(testFile, smallSpec, {
      id: "@test",
      version: "1.0.0",
    });
  ...
  it("should split large spec", () => {
    const splitter = new Splitter();
    const result = splitter.split(testFile, largeSpec, {
      id: "@test",
      version: "1.0.0",
    });
...
describe("DirectoryBuilder", () => {
  ...
  it("should check if path is spec dir", () => {
    expect(
      DirectoryBuilder.isSpecDir(path.join(tempDir, "test.spec.dir")),
    ).toBe(true);
...
describe("IndexUpdater", () => {
  ...
  it("should create index content", () => {
    const children = [
      {
        path: "specs/test.spec.dir/1.yaml",
        content: "",
        part: 1,
        totalParts: 2,
      },
      {
        path: "specs/test.spec.dir/2.yaml",
        content: "",
        part: 2,
        totalParts: 2,
      },
