...
import { TriggerRouter, InMemoryAgentRegistry } from '../src/cascade/triggers/router';
import {
  TriggerHandler,
  UserEditHandler,
  AgentWriteHandler,
...
import { TriggerWatcher, createWatcher, DEFAULT_WATCH_CONFIG } from '../src/cascade/triggers/watcher';
...
describe('Pattern Matching', () => {
  ...
  test('should match glob patterns with **', () => {
    ...
    expect(matchPattern('specs/nested/deep/file.scl', 'specs/**/*.scl')).toBe(true);
  });
...
describe('Integration Tests', () => {
  ...
  test('convergence detection', async () => {
    ...
    expect(manager.getCascade(result.cascadeStarted!)?.status).toBe('converged');
  });
});
