...
export class ConfidenceScorer {
  ...
  calculateConfidence(signals: SignalScore[]): { score: number; level: ConfidenceLevel } {
    const weightedSum = signals.reduce((sum, signal) => {
      const sourceWeight = SIGNAL_SOURCE_WEIGHTS[signal.signal_name] ?? 0.1;
  ...
  decideAction(
    ...
    isCriticalChange: boolean = false
  ): string {
    const level = this.determineLevel(confidence);
  ...
  generateReport(
    ...
    const signalDetails = signals.map((signal) => ({
      ...
      contributing_score:
        signal.value * (SIGNAL_SOURCE_WEIGHTS[signal.signal_name] ?? 0.1),
    }));
  ...
  getThresholds(): { proceed: number; fallback: number; abort: number } {
    return { ...this.thresholds };
  }

  getProjectLevel(): string {
  ...
  private loadThresholds(
    projectLevel: string
  ): { proceed: number; fallback: number; abort: number } {
