...
async function assembleOne(specFile: string, mirror: boolean = false): Promise<AssembleResult> {
  ...
  try {
    ...
    if (mirror) {
      try { await createMirrorSymlink(outputFile, specFile); } catch {}
    }
...
async function main() {
  ...
  if (mirror) console.log('  🪞 Mirror mode enabled\n');
  ...
  for (const file of files) {
    const result = await assembleOne(file, mirror);
    if (result.success) {
      console.log(`  ✅ ${file}`);
