...
export class SpecsToolHandler {
   ...
   */
  async handleGetSpec(args: GetSpecInput): Promise<{
    ...
    const row = db.prepare(specQuery).get(...params) as {
      file_path: string;
      id: string | null;
      tags: string;
    ...
    const metadata: MCPSpecMetadata = {
      id: row.id || '',
    ...
    const result: {
      metadata: MCPSpecMetadata;
      content?: string;
      blocks?: string[];
      dependencies?: string[];
      dependents?: string[];
    } = { metadata, blocks, dependents };
   ...
   */
  private getDependents(specId: string): string[] {
    const db = this.db.getDatabase();
    ...
    `).all(specId, specId) as Array<{ id: string }>;
   ...
   */
  async handleCreateSpec(args: CreateSpecInput): Promise<{
    success: boolean;
    file: string;
   ...
   */
  async handleUpdateSpec(args: UpdateSpecInput): Promise<{
    success: boolean;
    ...
    const existing = db.prepare(
      'SELECT * FROM specs WHERE id = ? OR file_path = ?'
    ).get(id, id) as {
      file_path: string;
      content_raw: string;
    } | undefined;
   ...
   */
  async handleListSpecs(args: ListSpecsInput): Promise<{
    ...
    const specs: MCPSpecMetadata[] = rows.map(row => ({
      id: row.id || '',
      file_path: row.file_path,
   ...
   */
  private validateSpecContent(content: string): ValidationResult {
    const errors: string[] = [];
   ...
   */
  private indexSpec(filePath: string, content: string, agentId?: string): void {
   ...
   */
  private createVersion(filePath: string, content: string, agentId?: string): void {
    const crypto = require('crypto');
    const hash = crypto.createHash('md5').update(content).digest('hex');
    
    const db = this.db.getDatabase();
    const spec = db.prepare('SELECT rowid FROM specs WHERE file_path = ?').get(filePath) as { rowid: number } | undefined;
