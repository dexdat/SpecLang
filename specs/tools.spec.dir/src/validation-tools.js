...
exports.validateSpecTool = exports.validateRefsTool = exports.validateHeaderTool = void 0;
 ...
 */
async function checkRefExists(ref, basePath = 'specs') {
        ...
        try {
            ...
            for (const ref of refs) {
                ...
                if (!exists && context.db) {
                    const refId = ref.replace('@ref:', '').split('#')[0];
                    const row = context.db.getDatabase().prepare('SELECT id FROM specs WHERE id = ?').get(refId);
                    exists = !!row;
                }
                if (!exists) {
                    brokenRefs.push(ref);
