...
exports.validateHeaderFields = validateHeaderFields;
...
function validateField(name, value) {
    const def = fields_1.FIELD_DEFINITIONS[name];
 ...
 */
function validateByType(def, value) {
    switch (def.type) {
        case 'id':
...
// TYPE VALIDATORS
// ============================================================================
function validateId(def, value) {
    if (typeof value !== 'string') {
        return makeError(def, 'INVALID_TYPE', `${def.name} must be a string`, value);
    }
    if (def.pattern && !def.pattern.test(value)) {
        return makeError(def, 'INVALID_ID_FORMAT', `Invalid id format: "${value}". Expected @domain/path (e.g. ${def.example})`, value);
    }
    return makeOk(def);
}
function validateSemver(def, value) {
    if (typeof value !== 'string' && typeof value !== 'number') {
        return makeError(def, 'INVALID_TYPE', `${def.name} must be a string`, value);
    }
    const strVal = String(value);
    if (!fields_1.SEMVER_PATTERN.test(strVal)) {
        return makeError(def, 'INVALID_SEMVER', `Invalid semver: "${strVal}". Expected format like ${def.example}`, value);
    }
    return makeOk(def);
}
function validateNumber(def, value) {
    ...
    if (def.range) {
        if (!Number.isInteger(value)) {
            return makeError(def, 'INVALID_NUMBER', `${def.name} must be an integer`, value);
        }
        if (value < def.range.min || value > def.range.max) {
            return makeError(def, 'OUT_OF_RANGE', `${def.name} must be between ${def.range.min} and ${def.range.max}, got ${value}`, value);
        }
    }
    return makeOk(def);
}
function validateEnum(def, value) {
    ...
    if (def.enumValues && !def.enumValues.includes(value)) {
        return makeError(def, 'INVALID_ENUM', `Invalid ${def.name}: "${value}". Valid values: ${def.enumValues.join(', ')}`, value);
    }
    return makeOk(def);
}
function validateString(def, value) {
    if (typeof value !== 'string') {
        return makeError(def, 'INVALID_TYPE', `${def.name} must be a string`, value);
    }
    if (def.pattern && !def.pattern.test(value)) {
        return makeError(def, 'INVALID_FORMAT', `${def.name} does not match expected format`, value);
    }
    return makeOk(def);
}
function validateStringArray(def, value) {
    if (!Array.isArray(value)) {
        return makeError(def, 'INVALID_TYPE', `${def.name} must be an array`, value);
    }
    for (let i = 0; i < value.length; i++) {
        if (typeof value[i] !== 'string') {
            return makeError(def, 'INVALID_ARRAY_ITEM', `${def.name}[${i}] must be a string, got ${typeof value[i]}`, value);
        }
    }
    return makeOk(def);
}
function validateRef(def, value) {
    if (typeof value !== 'string') {
        return makeError(def, 'INVALID_TYPE', `${def.name} must be a string`, value);
    }
    ...
    if (def.pattern && !def.pattern.test(normalized)) {
        return makeError(def, 'INVALID_REF_FORMAT', `Invalid reference format: "${value}". Expected @ref:path (e.g. ${def.example})`, value);
    }
    return makeOk(def);
}
function validateRefArray(def, value) {
    if (!Array.isArray(value)) {
        return makeError(def, 'INVALID_TYPE', `${def.name} must be an array`, value);
    }
    for (let i = 0; i < value.length; i++) {
        const item = value[i];
        const itemStr = typeof item === 'string' ? item : (item && typeof item === 'object' && 'ref' in item) ? item.ref : null;
        if (typeof itemStr !== 'string') {
            return makeError(def, 'INVALID_ARRAY_ITEM', `${def.name}[${i}] must be a string or {ref: string}`, value);
        }
        // Normalize and validate
        const normalized = itemStr.startsWith('@ref:') ? itemStr : `@ref:${itemStr}`;
        if (def.pattern && !def.pattern.test(normalized)) {
            return makeError(def, 'INVALID_REF_FORMAT', `Invalid reference in ${def.name}[${i}]: "${itemStr}"`, value);
        }
    }
    return makeOk(def);
}
function validatePart(def, value) {
    if (typeof value !== 'string') {
        return makeError(def, 'INVALID_TYPE', `${def.name} must be a string`, value);
    }
    if (!fields_1.PART_PATTERN.test(value)) {
        return makeError(def, 'INVALID_PART_FORMAT', `Invalid part format: "${value}". Expected N/M (e.g. ${def.example})`, value);
    }
    // Validate part <= total
    const [partNum, total] = value.split('/').map(Number);
    if (partNum < 1 || partNum > total) {
        return makeError(def, 'INVALID_PART_RANGE', `Part ${partNum} is out of range (1-${total})`, value);
    }
    return makeOk(def);
}
// ============================================================================
...
function validateHeaderFields(header) {
    ...
    const requiredNames = (0, fields_1.getRequiredFieldNames)();
...
function makeError(def, code, message, value) {
    return {
        field: def.name,
        valid: false,
        severity: 'error',
