...
parent: ""@ref:speclang/parser"part: 2/2
...
- Depth consistency (child >= parent)
...
```speclang
# @block:validation/depth @kind:entity
DepthValidation:
  validate_layer: "(layer) -> bool - Check depth integer"
  consistency_rules:
    - Child specs (referenced via parent) must have depth >= parent depth
    - Specs with depends_on should have similar depths (±2)
    - Code specs (depth 5) must reference implementation specs (depth 4)
    - Generated code (depth 6) must reference code specs (depth 5)
```
