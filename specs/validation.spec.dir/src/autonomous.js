    ...
    check(spec, _context) {
        ...
        for (const field of REQUIRED_AUTONOMOUS_FIELDS) {
            if (!spec.metadata[field]) {
                results.push((0, types_1.createError)('@validation/autonomous', { file: spec.filepath, line: 'header' }, `Autonomous spec missing required field: ${field}`, `Add "${field}: <value>" to the header`));
            }
        }
        ...
        for (const block of operationBlocks) {
            if (!hasStepByStepDescription(block)) {
                results.push((0, types_1.createWarning)('@validation/autonomous', { file: spec.filepath, line: block.line }, `Operation block ${block.id} lacks step-by-step description`, 'Add numbered steps or clear procedural description'));
            }
        }
        // No ambiguous language
        const ambiguousResults = detectAmbiguity(spec.content);
        for (const match of ambiguousResults) {
            results.push((0, types_1.createWarning)('@validation/autonomous', { file: spec.filepath, line: match.line }, `Potentially ambiguous language: "${match.text}"`, match.suggestion));
        }
        // Tags should not be empty for autonomous specs
        if (!spec.metadata.tags || spec.metadata.tags.length === 0) {
            results.push((0, types_1.createWarning)('@validation/autonomous', { file: spec.filepath, line: 'header' }, 'Autonomous spec should have tags for discoverability', 'Add tags: [tag1, tag2, ...]'));
        }
        // Layer should be specified (0-10)
        if (spec.metadata.layer === undefined) {
            results.push((0, types_1.createError)('@validation/autonomous', { file: spec.filepath, line: 'header' }, 'Autonomous spec must specify layer', 'Add "layer: <0-10>" to indicate abstraction level'));
        }
        // Verify that all references in blocks exist in the spec
        const allRefStrings = new Set();
        ...
        while ((match = contentRefRegex.exec(spec.content)) !== null) {
            ...
            if (!knownRefs.has(refStr) && !knownRefs.has(`@ref:${match[1]}`)) {
                results.push((0, types_1.createWarning)('@validation/autonomous', { file: spec.filepath, line: 'content' }, `Reference "${refStr}" in content but not declared in header`, `Add to depends_on or refs in header`));
            }
        }
        return results;
    },
 ...
 */
function hasStepByStepDescription(block) {
 ...
 */
function detectAmbiguity(content) {
    const results = [];
...
exports.default = exports.autonomousRule;
