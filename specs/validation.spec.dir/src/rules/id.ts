 ...
 */
export const idRule: ValidationRule = {
  id: '@validation/id',
  ...
  check(spec: ParsedSpec, _context?: ValidationContext): ValidationResult[] {
    ...
    if (domain !== domain.toLowerCase()) {
      results.push(createError(
        ...
        `Use "${domain.toLowerCase()}" instead`
      ));
    }
    ...
    if (id.includes('\\')) {
      results.push(createError(
        '@validation/id',
        { file: spec.filepath, line: 'header' },
        'ID must use forward slashes, not backslashes',
        'Use "/" instead of "\\"'
      ));
    }
    ...
    if (invalidChars) {
      ...
      results.push(createError(
        '@validation/id',
        { file: spec.filepath, line: 'header' },
        `Invalid characters in ID: ${uniqueInvalid.join(', ')}`,
    ...
    if (id.includes('//')) {
      results.push(createError(
        '@validation/id',
        { file: spec.filepath, line: 'header' },
        'ID contains empty path segment',
    ...
    if (id.length > 200) {
      results.push(createWarning(
        '@validation/id',
        { file: spec.filepath, line: 'header' },
        `ID is very long (${id.length} characters)`,
        'Consider a shorter, more readable ID'
      ));
    }
