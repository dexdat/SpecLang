...
export function resolveCollectionType(stdlibType: string): TypeResolution | null {
  ...
  if (frozenMatch) {
    const inner = resolvePythonType(frozenMatch[1]);
    return {
...
export function getCollectionDefault(stdlibType: string): string {
  if (stdlibType.includes('Array<') || stdlibType.includes('List<')) return '[]';
  if (stdlibType.includes('Map<') || stdlibType.includes('Dict<')) return '{}';
  if (stdlibType.includes('Set<')) return 'set()';
  if (stdlibType.includes('FrozenSet<')) return 'frozenset()';
  if (stdlibType.includes('Tuple<')) return '()';
  return '[]';
}
