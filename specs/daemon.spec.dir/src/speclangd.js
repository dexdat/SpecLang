...
program
    ...
    const daemon = new index_js_1.Daemon(options.config);
    daemon.on('started', () => {
...
program
    ...
    .action(async () => {
    const daemon = new index_js_1.Daemon();
    await daemon.start();
...
program
    ...
    const daemon = new index_js_1.Daemon();
    await daemon.start();
    await daemon.processCommand({ kind: index_js_1.DaemonCommandKind.Pause });
...
program
    ...
    const daemon = new index_js_1.Daemon();
    await daemon.start();
    await daemon.processCommand({ kind: index_js_1.DaemonCommandKind.Resume });
...
program
    ...
    const daemon = new index_js_1.Daemon();
    await daemon.start();
    await daemon.processCommand({ kind: index_js_1.DaemonCommandKind.Abort });
...
program
    ...
    const daemon = new index_js_1.Daemon();
    await daemon.start();
    await daemon.processCommand({ kind: index_js_1.DaemonCommandKind.Trigger, path: file });
...
//# sourceMappingURL=speclangd.js.map