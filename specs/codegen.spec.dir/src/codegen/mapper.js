...
exports.getStdlibTypes = getStdlibTypes;
exports.isStdlibType = isStdlibType;
...
function mapType(stdlibType, target) {
    // Handle generic types
    if (stdlibType.includes('<')) {
        return mapGenericType(stdlibType, target);
    }
    // Find exact match
    const mapping = exports.TYPE_MAPPINGS.find(m => m.stdlib === stdlibType);
    if (mapping) {
...
function mapGenericType(stdlibType, target) {
...
function toRustType(str) {
    // Handle primitives
    const rustPrimitives = {
        string: 'String',
        number: 'i32',
        boolean: 'bool',
...
function getStdlibTypes() {
    return exports.TYPE_MAPPINGS.map(m => m.stdlib);
}
/** Check if a type is a valid stdlib type */
function isStdlibType(type) {
    return exports.TYPE_MAPPINGS.some(m => m.stdlib === type);
}
/** Get mapping for a specific type */
function getTypeMapping(stdlibType) {
    return exports.TYPE_MAPPINGS.find(m => m.stdlib === stdlibType);
}
