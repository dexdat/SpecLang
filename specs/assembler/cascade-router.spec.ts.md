---
id: "@speclang/assembler/cascade-router"
version: 1.0.0
layer: 2
target_lang: ts
output: .speclang/cascade-router.spec.ts
owned-by: assembler
model_pool: code-gen
max_concurrent: 1
seed: false
tags: [assembler, cascade, router, throttle, squash, model-pool]
short: "Cascade router — event-to-agent dispatcher with throttle/squash and model pool dispatch"
depends_on:
  - "@ref:specs/cascade"
...
The cascade router receives FileChangeEvents from speclangd, applies squash (debounce) and throttle (fairness deferral), resolves model/pool from spec headers, checks rate limits, and dispatches Pi agent sessions.

### Architecture

```
FileChangeEvent from speclangd
       |
       v
+------------------+
| Squash           |  Debounce rapid changes to same file (100ms)
+------------------+
       |
       v
+------------------+
| Dependency       |  Build list of affected specs from notification graph
| Resolver         |
+------------------+
       |
       v
...
|                  |  Defer, requeue, backoff, notify at 3+
+------------------+
       |
       v
+------------------+
| Model Pool       |  Read header model/model_pool, resolve 3 layers
| Resolver         |  Check rate limits (spec + pool + provider)
+------------------+
       |
       v
+------------------+
| Pi Session       |  Spawn Pi agent sessions via createAgentSession()
| Dispatcher       |  Load skills, pass context, subscribe to events
+------------------+
```
...
# Unit test squash
npx tsx -e "
const { SquashBuffer } = require('./.speclang/cascade-router.spec.ts');
// Test debounce
"

# Unit test throttle
npx tsx -e "
const { ThrottleController } = require('./.speclang/cascade-router.spec.ts');
const t = new ThrottleController(3, 10); // 3 hits in 10s
t.recordQueue('hot-file.spec.md');
t.recordQueue('hot-file.spec.md');
t.recordQueue('hot-file.spec.md');
console.log('Is hot:', t.isHot('hot-file.spec.md')); // true
"
```
