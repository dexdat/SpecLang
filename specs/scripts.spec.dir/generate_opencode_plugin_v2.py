...
# target: scripts/generate_opencode_plugin_v2.py
#!/usr/bin/env python3
"""
...
and writes them to src/opencode-plugin/index.ts with fixes for common issues.
"""
...
def fix_code_blocks(blocks):
    ...
    for spec_name, block_id, code in blocks:
        # Replace db.run(...) with db.prepare(...).run()
        code = re.sub(r'\bdb\.run\(', 'db.prepare(', code)
        # After replacement, need to add .run() if not already there
        # This is simplistic; better to do more sophisticated transformation
        # For now, we'll just replace db.run with db.prepare and assume .run() is called later
        # Actually better to leave as is and declare db as any
...
def main():
    ...
    output_lines.append("import * as fs from 'fs';")
    ...
    output_lines.append("import { exec } from 'child_process';")
    output_lines.append("import { promisify } from 'util';")
    output_lines.append('')
    ...
    output_lines.append('declare const db: any;')
