---
id: "@speclang/health/core"
version: 1.0.0
layer: 1
target_lang: ts
output: specs/health/assembled/health-core.spec.ts
owned-by: health
status: active
tags: [health, metrics, scanner, dashboard]
---

# Health Core — Spec Indexer & Metrics

## Overview

Core library for scanning SpecLang spec files and computing health metrics.

### Architecture

```
scanSpecs(dir) → SpecInfo[]
computeHealth(specs) → HealthReport
...
```bash
npx tsx -e "
const { scanSpecs, computeHealth, formatReport } = require('./specs/health/assembled/health-core.spec.ts');
scanSpecs().then(specs => console.log(formatReport(computeHealth(specs))));
"
```
