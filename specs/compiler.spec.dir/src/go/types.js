...
exports.mapGoType = mapGoType;
exports.getGoZeroValue = getGoZeroValue;
...
function mapGoType(stdlibType) {
    ...
    const mapping = exports.GO_TYPE_MAPPINGS.find((m) => m.stdlib === stdlibType);
    if (mapping) {
        return {
            type: mapping.go,
...
function getGoZeroValue(stdlibType) {
    const arrayMatch = stdlibType.match(/^Array<(.+)>$/);
    if (arrayMatch)
        return 'nil';
    const mapMatch = stdlibType.match(/^Map<(.+),\s*(.+)>$/);
