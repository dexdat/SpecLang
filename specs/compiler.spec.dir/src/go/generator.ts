...
export class GoCodeGenerator {
  ...
  generateBlockParts(block: SpecBlock): { bodyParts: string[]; imports: string[] } {
    const parts: string[] = [];
    ...
    return { bodyParts: parts, imports: allImports };
  }

  generate(block: SpecBlock): GeneratedGoCode {
    const pkg = block.package || this.options.packageName || 'generated';
    const { bodyParts, imports } = this.generateBlockParts(block);
    const body = bodyParts.join('\n');
    const code = renderFile(pkg, imports, body, block.name);
...
export interface GoPackageResult {
  pkgName: string;
  files: GeneratedGoCode[];
}
...
export class GoPackageGenerator {
  ...
  generateFromBlocks(blocks: SpecBlock[], pkgName: string): GoPackageResult {
    const fileGroups = new Map<string, { bodies: string[]; imports: Set<string> }>();
    ...
    for (const block of blocks) {
      ...
      if (block.kind === 'entity') {
        ...
        source = 'service.go';
      } else if (block.name.includes('Error')) {
        source = 'errors.go';
      } else {
        source = `${toSnakeCase(block.name)}.go`;
      }

      if (!fileGroups.has(source)) {
        fileGroups.set(source, { bodies: [], imports: new Set<string>() });
      }

      const group = fileGroups.get(source)!;
      const { bodyParts, imports: blockImports } = this.generator.generateBlockParts(block);

      if (bodyParts.length > 0) {
        group.bodies.push(bodyParts.join('\n'));
      }
      for (const imp of blockImports) {
        group.imports.add(imp);
      }
    }

    const files: GeneratedGoCode[] = [];
    for (const [source, group] of fileGroups) {
      if (group.bodies.length === 0) continue;
      const imports = Array.from(group.imports).sort();
      const body = group.bodies.join('\n\n');
      const code = renderFile(pkgName, imports, body, source);
      files.push({ code, imports, package: pkgName });
    }
