...
import { getLevelOrder, isValidTransition, getLevelDefinition } from './levels';
...
export class TransitionManager {
  ...
  getAvailableTransitions(from: MaturityLevel): MaturityLevel[] {
    const transitions: MaturityLevel[] = [];
   ...
   * Run a single check
   */
  private runCheck(spec: ParsedSpec, check: TransitionCheck): CheckResult {
    let passed = false;
    let message = '';
   ...
   */
  private checkDocumentation(spec: ParsedSpec, description: string): boolean {
   ...
   */
  private checkTesting(spec: ParsedSpec, description: string): boolean {
   ...
   */
  private checkDeployment(spec: ParsedSpec, description: string): boolean {
   ...
   */
  private scoreDocumentation(spec: ParsedSpec): number {
    let score = 0;
    const m = spec.metadata;
    if (m.id) score += 15;
    if (m.version) score += 15;
    if (m.short) score += 15;
    if (m.layer !== undefined) score += 10;
    if (m.tags?.length) score += 10;
    if (m.status) score += 10;
    if (m.project_level) score += 10;
    if (m.agent_support) score += 15;
    return score;
  }
