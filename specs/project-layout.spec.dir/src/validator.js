...
function validateProject(projectRoot) {
    ...
    if (projectRoot) {
        ...
        root = (0, config_js_1.findProjectRoot)();
    }
    if (!root) {
        return {
            valid: false,
            issues: [{
                    severity: types_js_1.ValidationSeverity.Error,
    ...
    const structure = issues.filter(i => i.severity === types_js_1.ValidationSeverity.Error).length === 0
        ? (0, config_js_1.buildProjectStructure)(root)
        : undefined;
    return {
        valid: issues.filter(i => i.severity === types_js_1.ValidationSeverity.Error).length === 0,
        issues,
        structure
    };
}
 ...
 */
function performValidationChecks(projectRoot) {
    return {
...
function collectIssues(checks) {
    ...
    if (!checks.hasGeneratedDir) {
        issues.push({
            severity: types_js_1.ValidationSeverity.Warning,
    ...
    if (checks.specFiles.length === 0) {
        issues.push({
            severity: types_js_1.ValidationSeverity.Info,
            code: 'NO_SPEC_FILES',
 ...
 */
function getSpecFiles(projectRoot) {
    ...
    try {
        ...
        return files.filter(f => f.endsWith('.scl') ||
            f.endsWith('.spec.md') ||
 ...
 */
function getTestFiles(projectRoot) {
 ...
 */
function isProjectValid(projectRoot) {
...
function getValidationSummary(result) {
    const errors = result.issues.filter(i => i.severity === types_js_1.ValidationSeverity.Error);
    const warnings = result.issues.filter(i => i.severity === types_js_1.ValidationSeverity.Warning);
    const infos = result.issues.filter(i => i.severity === types_js_1.ValidationSeverity.Info);
    const lines = [];
    if (result.valid) {
