...
exports.extractRefsFromContent = extractRefsFromContent;
exports.extractBlocksFromContent = extractBlocksFromContent;
exports.getSpecFiles = getSpecFiles;
exports.generateIndex = generateIndex;
exports.populateDatabase = populateDatabase;
exports.validateIndexCmd = validateIndexCmd;
exports.treeCmd = treeCmd;
exports.impactCmd = impactCmd;
...
function parseHeader(filepathOrContent) {
    let content;
...
function extractBasicFields(yamlText) {
    const metadata = {};
    ...
    if (layerMatch)
        metadata.layer = parseInt(layerMatch[1], 10);
    return metadata;
}
// ============================================================================
...
function extractRefsFromContent(filepathOrContent) {
    let content;
    // Check if it's a file path or content
...
function extractBlocksFromContent(filepathOrContent) {
    let content;
    // Check if it's a file path or content
    if (filepathOrContent.includes('@ref:') || filepathOrContent.includes('@block:')) {
        // It's content
    ...
    function walk(dir) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                ...
                if (relPath.includes('.backup_spec_files'))
                    continue;
                let isSpec = false;
...
function generateIndex(options = {}) {
    const opts = { ...types_1.DEFAULT_INDEXER_OPTIONS, ...options };
        ...
        try {
            ...
            const layer = typeof layerRaw === 'number'
                ...
                : typeof layerRaw === 'string' && /^\d+$/.test(layerRaw)
                    ...
                    : 0;
            const specId = metadata.id || `@unknown/${path.basename(relPath)}`;
            allIds.add(specId);
            const entry = {
                id: specId,
                file: relPath,
    ...
    // Build graphs
    const { dependencies, dependents } = (0, graph_1.buildDependencyGraph)(entries);
    // Detect cycles
    const cycles = (0, graph_1.detectCycles)(dependencies);
    // Find orphans
    const orphans = (0, graph_1.findOrphans)(dependencies, dependents, allIds);
    // Validate refs
...
/** Validate references */
function validateRefs(entries, allIds) {
    const missing = [];
    const valid = [];
    // Normalize allIds
 ...
 */
function populateDatabase(index, db) {
 ...
 * Validate index
 */
function validateIndexCmd(index) {
 ...
 */
function treeCmd(index, specId) {
    const deps = index.graph.dependencies;
    const dents = index.graph.dependents;
    console.log(`=== Dependency Tree for ${specId} ===`);
    const transitiveDeps = (0, graph_1.getTransitiveDependencies)(specId, deps);
    const transitiveDents = (0, graph_1.getTransitiveDependents)(specId, dents);
