# Bootstrap Phase 0.36: Project Level - MVP (Minimum Viable Product)

## Context
You are building SpecLang - a reactive multi-agent system where specs self-assemble into code. This is Phase 0.36 of the bootstrap process.

**Prerequisites**: 
- Phase 0.35 (POC Level) complete

## Your Task
Implement the MVP (Minimum Viable Product) project level - the second maturity stage. MVP represents core functionality validated and ready for early adopters.
...
3. `docs/prompts/phase-0.35-project-levels-poc.md` - POC implementation
...
```
src/maturity/levels/
├── mvp.ts                  # MVP level definition
├── mvp-validator.ts        # MVP-specific validation
├── mvp-transitions.ts      # MVP transition handling
└── mvp-behavior.ts        # MVP agent behavior

tests/maturity/
├── mvp.test.ts
└── mvp-validator.test.ts
```

### Requirements

#### 1. MVP Level Definition (mvp.ts)
```typescript
...
#### 2. MVP Validator (mvp-validator.ts)
```typescript
...
#### 3. MVP Transitions (mvp-transitions.ts)
```typescript
...
#### 4. MVP Agent Behavior (mvp-behavior.ts)
```typescript
import { MVP_LEVEL } from './mvp';
...
## Test Cases
1. MVP level requires id, version, tags, short
2. MVP validator detects production deployment target as error
3. MVP validator detects layer > 4 as error
4. MVP can transition from POC with checklist
5. MVP can transition to Alpha with checklist
6. MVP agent behavior allows code generation with review
7. MVP agent behavior allows internal/staging deployment
8. MVP validator calculates readiness score
9. MVP suggestions provided to agents

## CLI Commands
```bash
# Check if spec is valid MVP
speclang level --check-mvp specs/auth.spec.md

# Get MVP transition checklist from POC
speclang level --transition-checklist POC->MVP specs/auth.spec.md

# Get MVP transition checklist to Alpha
speclang level --transition-checklist MVP->Alpha specs/auth.spec.md

# Get MVP readiness score
speclang level --readiness specs/auth.spec.md

# Get MVP agent behavior
speclang level --behavior MVP
```

## Validation
```bash
bun test tests/maturity/mvp.test.ts
```

## Output Format
After completing, output:
1. MVP level definition with criteria
2. MVP validator implementation
3. Transition checklists (POC->MVP, MVP->Alpha)
