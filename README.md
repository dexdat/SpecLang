...
- Dual-view compliance: 99.2% — 596 files follow the specs→working-location pattern (5 legacy `scripts/archive/` scripts lack spec sources; 160 exempt; see `scripts/check_compliance.py --report`, 2026-08-05)
- 1822 tests passing (58 skipped; verified 2026-08-11; known flake: arch004 cascade convergence timeout)
...
# The spec index (_index.json) is regenerated by the test suite; it currently
# holds 468 entries (467 real specs + 1 test artifact from .tmp/; verified
# 2026-08-11). A bare `python3 scripts/generate_index.py` prints usage — it
# requires a mode flag (--validate/--tree/--impact/--graph). There is no
# `speclang index` CLI command registered in the binary yet.
```
