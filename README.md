...
# holds 466 entries (verified 2026-08-13). The generator is explicit-mode
# only: a bare `python3 scripts/generate_index.py` prints usage and exits
# without writing anything. Use `--generate` to (re)write the index, or
# `--validate`/`--tree`/`--impact`/`--graph` for read-only analysis.
# `--validate` exits non-zero when missing references exceed `--max-missing`
# (default 0) or when circular dependencies exist; `--max-missing N` is the
# gate tunable. There is no `speclang index` CLI command registered in the
# binary yet.
```
