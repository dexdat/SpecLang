...
async function preProcessSpec(specPath: string, targetLang: string, projectRoot: string): Promise<string | null> {
  try {
    ...
    const traceHeaders = [
      `// spec:trace: ${specPath}#${sectionAnchor}`,
    ...
    try {
      ...
      const stageInstructions: Record<string, string> = {
        thinker: [
          ...
          `   > **spec:trace:** specs/auth.spec.md#auth-strategy L12-34`,
          `   > **spec:requires:** specs/security.spec.md#token-format L45-67`,
          ...
          `<!-- spec:trace: specs/auth.spec.md#login-flow L45-89 | specs/security.spec.md#password-hash L90-112 -->`,
          ``,
          `### QUALITY GATES`,
        ...
        assembler: [
          ...
          `// spec:trace: ${resolvedPath}#implementation L<start>-<end>`,
        ...
        codegen: [
          ...
          `// spec:trace: <source-spec>#<section> L<lines> → <target-lang-file>`,
          `// spec:id: {spec identifier}`,
          `// spec:cascade: ${item.cascadeId}`,
          `// spec:satisfies: <AC-ID>`,
          `// spec:implements: @kind:operation <name>`,
          ``,
          `### QUALITY GATES`,
        ...
        testwriter: [
          ...
          `// spec:trace: ${resolvedPath}#<section> L<lines>`,
          `// spec:tests: <operation-name>, <edge-case>`,
          ``,
          `### QUALITY GATES`,
      ...
      const prompt = [
        ...
        ``,
        `SpecLang traces create a FULLY AUDITABLE, BIDIRECTIONAL chain:`,
        `  Requirement → Design → Implementation → Verification`,
        ``,
        `Every generated artifact MUST carry trace annotations. This is NOT optional.`,
        `A model reading a code file should be able to follow the trace BACK to the exact`,
        `section of the spec that produced it, including which AC/UC it satisfies.`,
        ``,
        `### Section Anchors (HTML #fragment)`,
        ``,
        `All spec:trace paths use markdown heading anchors so models can jump to exact sections:`,
        `  specs/auth.spec.md#login-flow          — the ## Login Flow section`,
        `  specs/auth.spec.md#password-hash       — the ### Password Hash subsection`,
        `  specs/core.spec.md#cascade-routing     — the ## Cascade Routing section`,
        ``,
        `Anchors are derived from markdown headings by:`,
        `  1. Lowercase the heading text`,
        `  2. Replace spaces and punctuation with hyphens`,
        `  3. Strip leading/trailing non-alphanumeric chars`,
        `  Example: "## User Authentication Flow" → #user-authentication-flow`,
        ``,
        `### The Trace Chain (Bidirectional)`,
        ``,
        `Every piece of code carries FORWARD traces (where it came from):`,
        `  // spec:trace: specs/auth.spec.md#login-flow L45-89`,
        `  // spec:requires: specs/auth.spec.md#password-hash L90-112`,
        `  // spec:satisfies: AC-003 (user authentication)`,
        `  // spec:implements: @kind:operation loginUser`,
        ``,
        `Every spec carries REVERSE traces (what implements it):`,
        `  ## Generated → src/auth/login.ts L23-67`,
        `  ## Verified by → tests/auth/login.test.ts L12-34`,
        ``,
        `### Trace Annotation Reference`,
        ``,
        `| Annotation | Meaning | Example |`,
        `|-----------|---------|---------|`,
        `| spec:trace | Primary source spec + section | specs/auth.spec.md#login-flow L45-89 |`,
        `| spec:requires | Dependency — this code needs another spec section | specs/auth.spec.md#password-hash L90-112 |`,
        `| spec:satisfies | Acceptance Criteria this code fulfills | AC-003 (user authentication) |`,
        `| spec:implements | The @kind:operation or @block: this code implements | @kind:operation loginUser |`,
        `| spec:id | Spec identifier from header | @specs/auth |`,
        `| spec:cascade | Which cascade run produced this | cascade-${item.cascadeId} |`,
        `| spec:generated | Marker — do not hand-edit this file | DO NOT EDIT — edit the spec instead |`,
        `| spec:tests | Which operations this test covers | login-flow, error-paths, edge-cases |`,
        ``,
        `### Trace Formats by Output Type`,
        ``,
        `1. **Markdown thinking files** (plan, decision, context, pattern):`,
        ``,
        `   Every section header gets a reverse trace block:`,
        ``,
        `   ## Architecture Decision: Use JWT over Session Tokens`,
        `   > **spec:trace:** specs/auth.spec.md#auth-strategy L12-34`,
        `   > **spec:requires:** specs/security.spec.md#token-format L45-67`,
        `   > **Confidence:** HIGH (3 sources: auth.spec.md, security.spec.md, project.scl)`,
        `   > **Sources:**`,
        `   >   - specs/auth.spec.md#auth-strategy L12-34 — authentication approach comparison`,
        `   >   - specs/security.spec.md#token-format L45-67 — JWT structure requirements`,
        `   >   - specs/project.scl#auth-component — architecture constraints`,
        ``,
        `   At the bottom of each thinking file, add a reverse trace section:`,
        ``,
        `   ## Trace Index`,
        `   - specs/auth.spec.md#auth-strategy → this file §Architecture Decision`,
        `   - specs/security.spec.md#token-format → this file §Token Structure`,
        ``,
        ...
        `   // spec:cascade: ${item.cascadeId}`,
        ``,
        `   Per-function/class traces:`,
        `   // spec:trace: specs/auth.spec.md#login-flow L67-89`,
        `   // spec:satisfies: AC-003`,
        `   // spec:implements: @kind:operation loginUser`,
        ...
        ``,
        `   // spec:trace: specs/math.spec.ts.md#implementation L16-24`,
        ...
        `   File header:`,
        `   // spec:trace: specs/auth.spec.md#login-flow L45-89`,
        `   // spec:tests: login-flow, error-paths, edge-cases`,
        ``,
        `   Per-test traces:`,
        `   // spec:trace: specs/auth.spec.md#login-flow L67-89`,
        `   // spec:tests: invalid-credentials`,
        `   test('returns 401 for invalid credentials', async () => {`,
        ``,
        `### Reverse Traces (Spec → Code)`,
        ``,
        `After generating code, UPDATE the source spec with reverse traces so future`,
        `cascade agents can find existing implementations:`,
        ``,
        `  ## Implementation`,
        `  \`\`\`ts`,
        ...
        `  \`\`\``,
        ``,
        `  ## Generated → src/math.ts L1-3 (assembler stage)`,
        `  ## Verified by → tests/math.test.ts L5-10`,
        ``,
        `### Trace Confidence Rules`,
        `- **HIGH**: Section-level trace with line numbers. At least 2 independent sources confirm.`,
        `- **MEDIUM**: File-level trace only (no section anchor). Single source or partial agreement.`,
        `- **LOW**: Extrapolated from convention. No direct spec section. Flag for human review.`,
        `- NEVER mark MEDIUM as HIGH. A trace without a section anchor is NEVER HIGH confidence.`,
        ``,
        `### Why Traces Matter`,
        ``,
        `When a model reads a code file and sees:`,
        `  // spec:trace: specs/auth.spec.md#login-flow L67-89`,
        `  // spec:satisfies: AC-003`,
        ``,
        `It can immediately:`,
        `  1. read() the exact spec section that produced this code`,
        `  2. Understand WHY this code exists (the requirement)`,
        `  3. Find the AC that validates it`,
        `  4. Know whether to update or replace it`,
        ``,
        `Without traces, every model starts from scratch guessing what code does.`,
        `With traces, every model has full context to make informed decisions.`,
        ``,
