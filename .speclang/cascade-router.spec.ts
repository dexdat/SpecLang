  ...
  private async processItem(item: CascadeItem): Promise<void> {
    ...
    try {
      ...
      const prompt = [
        ...
        `3. If a git diff is shown above, it tells you EXACTLY what changed — only update code affected`,
        ``,
        `## CRITICAL: How to extract code from SpecLang specs`,
        `The spec contains blocks like this:`,
        `  \\\`\\\`\\\`speclang`,
        `  # @block:rewriter/result @kind:operation`,
        `  @speclang`,
        `  @dataclass`,
        ...
        ``,
        `TRANSLATION RULES:`,
        `- STRIP \`@speclang\` and \`@dataclass\` annotations — they are SpecLang DSL, not Python`,
        `- \`@dataclass\` → use Python \`@dataclass\` from \`dataclasses\` module, OR use \`pydantic.BaseModel\``,
        `- \`@pydantic\` → use \`pydantic.BaseModel\``,
        `- \`@block:name @kind:operation\` means "this is an implementation block" → generate actual code for it`,
        `- \`@block:name @kind:code\` → the content IS the code, just strip annotations and write`,
        `- \`@kind:note\` blocks → explanatory text, NOT code — include as docstrings`,
        `- ALL generated code must have proper imports, type hints, and follow the skill conventions`,
        ``,
        `4. Write the generated ${targetLang} code to ${outputDir}/`,
        `   - Use the component name from the spec (e.g., spec id @spec/chimera/rewriter → rewriter.py)`,
        `   - Follow the project's directory structure`,
        ``,
        `5. Run tests to verify: \`cd ${projectRoot} && ${targetLang === 'py' ? 'pip install -e ".[dev]" 2>/dev/null; python -m pytest tests/ -x -q 2>&1 | tail -20' : targetLang === 'ts' ? 'npm test 2>&1 | tail -20' : 'make test 2>&1 | tail -20'}\``,
        `6. Report: which files you created, test results, any issues`,
      ].filter(Boolean).join('\n');
