...
export class NotificationGraph {
  ...
  addSpec(specPath: string, header: SpecHeader, bodyRefs: string[] = []): void {
    ...
    // From @ref: annotations in spec body
...
/**
 * Extract @ref: annotations from a spec body.
 * Matches patterns like:
 *   @ref: specs/rewriter.spec.py.md
 *   @ref: project.scl → ## Components
 *   - @ref:northstar
 */
export function extractBodyRefs(body: string, specDir: string): string[] {
  const refs: string[] = [];
  const refRegex = /(?:^|\s)@ref:\s*([^\s\n→]+)/gm;
  let match;
  while ((match = refRegex.exec(body)) !== null) {
    let ref = match[1].trim();
    // Resolve relative refs to absolute paths
    if (!ref.startsWith('/') && !ref.startsWith('specs/')) {
...
/**
 * Parse both header and body from a spec file.
 */
export async function parseSpecFull(filePath: string): Promise<{ header: SpecHeader | null; bodyRefs: string[] } | null> {
  try {
    const content = await fs.readFile(filePath, 'utf-8');
    const match = content.match(/^---\n(.*?)\n---\n(.*)$/s);
    if (!match) return null;
    const header = yaml.load(match[1]) as SpecHeader;
    const bodyRefs = extractBodyRefs(match[2], path.dirname(filePath));
    return { header, bodyRefs };
  } catch {
    return null;
  }
}
...
export class SpeclangDaemon extends EventEmitter {
  ...
  private async indexExistingSpecs(): Promise<void> {
    ...
    const specFiles = await glob(['**/*.spec.md', '**/*.spec.*.md'], { cwd: this.watchPath });
    this.log.info('indexing existing specs', { count: specFiles.length });
    let indexed = 0, skipped = 0, totalEdges = 0;
    for (const file of specFiles) {
      const fullPath = path.join(this.watchPath, file);
      const parsed = await parseSpecFull(fullPath);
      if (parsed && parsed.header) {
        this.graph.addSpec(fullPath, parsed.header, parsed.bodyRefs);
        totalEdges += parsed.bodyRefs.length;
        this.graphLog.debug('indexed spec', { file, specId: parsed.header.id, bodyRefs: parsed.bodyRefs.length });
        indexed++;
      } else {
        skipped++;
  ...
  private async handleChange(filePath: string, kind: 'create' | 'modify' | 'delete'): Promise<void> {
    ...
    if (kind === 'delete') {
      ...
      const parsed = await parseSpecFull(filePath);
      if (parsed && parsed.header) {
        this.graph.addSpec(filePath, parsed.header, parsed.bodyRefs);
        this.graphLog.info('spec added/updated in graph', {
          path: filePath,
          specId: parsed.header.id,
          dependsOn: parsed.header.dependsOn,
          bodyRefs: parsed.bodyRefs.length,
        });
      }
    }
